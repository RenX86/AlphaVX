...
class ResultCache:
    ...
    def has(self, variant_key: str) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT 1 FROM variant_scores WHERE variant_key = ? AND config_hash = ?",
    ...
    def get(self, variant_key: str) -> dict | None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT scores_json FROM variant_scores WHERE variant_key = ? AND config_hash = ?",
                (variant_key, self.config_hash),
            )
            row = cursor.fetchone()
            if row is None:
    ...
    def put(self, variant_key: str, scores: dict) -> None:
        ...
        with self._lock:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute(
                    "DELETE FROM variant_scores WHERE variant_key = ? AND config_hash IS ?",
                    (variant_key, self.config_hash),
                )
                conn.execute(
